...
OVERVIEW_OUTPUT = ROOT / "niulai-open-frame-overview-v0.1.png"
CHASSIS_OUTPUT = ROOT / "niulai-open-frame-chassis-v0.1.png"
HEAD_OUTPUT = ROOT / "niulai-open-frame-head-mechanism-v0.1.png"

# Keep the original links usable, but replace both confusing legacy renders.
LEGACY_OVERVIEW_OUTPUT = ROOT / "niulai-open-frame-v0.1.png"
LEGACY_DETAIL_OUTPUT = ROOT / "niulai-open-frame-mechanisms-v0.1.png"
...
GROUP_OBJECTS = {"PrintedParts", "HardwarePlaceholders", "ReservedInterfaces"}
...
def save_overview(meshes: dict[str, dict]) -> None:
    ...
    ax.set_title("牛来机器人 P0｜开架机电骨架总览", fontsize=18, pad=20, weight="bold")
    ...
    fig.text(0.5, 0.025, "无外壳工程骨架｜占位尺寸，实物到货后复测孔位", ha="center", color="#8b2c1d", fontsize=11, weight="bold")
...
def save_chassis(meshes: dict[str, dict]) -> None:
    ...
    ax.set_title("底盘动力爆炸图｜两驱 + 电子层 + 后滚珠 + 前防倾", fontsize=17, pad=18, weight="bold")
...
def save_head(meshes: dict[str, dict]) -> None:
    ...
    ax.set_title("头部三执行器爆炸图｜PAN / TILT / MOUTH", fontsize=17, pad=18, weight="bold")
